...
public class Percolation {
    ...
    public void open(int row, int col) {
        if (row < 1 || col < 1 || row > this.width || col > this.width) throw new IllegalArgumentException();
        ...
        this.union(row, col, -1, 0);
        this.union(row, col, 1, 0);
        this.union(row, col, 0, -1);
        this.union(row, col, 0, 1);
    }

    private void union(int row, int col, int offr, int offc) {
        // is (row, col) in bounds
        if (    (0 < (row + offr) //above
                && (row + offr) <= width) //bellow
                && (0 < (col + offc) //left
                && (col + offc) <= width) //right
                && this.isOpen((row + offr), (col + offc))
        ) {
    ...
    public boolean percolates() {
        // 0 -> top index
        return this.topBottomVIndices.connected(this.bottomIndex, 0);
    }
